...
public class GetUsersActiveOrders {
    private final OrderRepository orderRepository;
    private final GetLoggedInUser getLoggedInUser;

    public List<OrderResponse> execute() {
        User user = getLoggedInUser.execute();
        return orderRepository.findAllByCustomerAndStatusNotIn(user, Arrays.asList(OrderStatus.DELIVERED, OrderStatus.REJECTED, OrderStatus.UNSUCCESSFUL_DELIVERY))
                .stream()
