...
public class MarkUnsuccessfulOrder {
    private final ChangeOrderStatus changeOrderStatus;
    private final SaveOrder saveOrder;

    public Order execute(UUID id, String reason) {
        Order order = changeOrderStatus.execute(id, OrderStatus.UNSUCCESSFUL_DELIVERY);
        order.setUnsuccessfulDeliveryReason(reason);
